# The Tech Alchemy Lab

The agency website for The Tech Alchemy Lab, rebuilt with the same modern stack used across its full-stack products.

## Current phase

Phase 5 adds the founder story, philosophy, six operating values and an honest 2014–2026 journey timeline. The original static implementation remains in `legacy/` as a content reference while each remaining section is migrated and verified.

## Stack

- Next.js 16 with App Router
- React 19
- TypeScript
- Tailwind CSS 4
- Vercel

Resend and persistent enquiry storage will be added during the dedicated contact-form phase.

## Local development

```powershell
npm install
npm run dev
```

Open `http://localhost:3000`.

## License

Copyright © 2026 Keketso Leu. All rights reserved. This source code is available for portfolio review only. No permission is granted to copy, modify, distribute, sublicense, or use it commercially without written permission.
